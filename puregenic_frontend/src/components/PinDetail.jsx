...
const PinDetail = ({ user }) => {
  ...
  const [comments, setComments] = useState(pinDetail?.comments);
    ...
    if (comment) {
      ...
      client
        ...
        .then(() => {
          ...
          setComments((prevComments) => [...prevComments, { comment, _key: uuidv4(), postedBy: { _type: 'postedBy', _ref: user._id } }]);
  ...
  return (
    <>
        ...
        <div className='w-full p-5 flex-1 xl:min-w-620'>
          <div className='flex items-center justify-between'>
            <div className='flex gap-2 items-center'>
              <a href={`${pinDetail?.asset?.url}?dl=`} download
          ...
          <div className='flex flex-wrap mt-6 gap-3'>
            ...
            <button
              ...
              onClick={addComment}>
              {addingComment ? 'Transmiting your thoughts...' : 'Comment'}
            </button>
          </div>
        </div>
      </div >
      {pins?.length > 0 ? (
        <>
          <h2 className='text-center font-bold text-2x mt-8 mb-4'>Like that pin?</h2>
